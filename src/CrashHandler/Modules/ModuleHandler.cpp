...
namespace Crash::Modules
{
	namespace detail
	{
		...
		private:
			...
			/* [[nodiscard]] static auto complete_object_locator(
				const Crash::msvc::type_info* a_typeDesc,
				std::span<const std::byte> a_module,
				std::span<const std::byte> a_rdata)
				-> const Crash::msvc::type_info*
			{
				...
				for (auto iter = start; iter < end; ++iter) {
					if (*iter == rva) {
						...
						const auto col = reinterpret_cast<const RTTI::CompleteObjectLocator*>(ptr - offsetof(RTTI::CompleteObjectLocator, typeDescriptor));
						if (col->offset != 0) {
							continue;
						}
			...
			[[nodiscard]] static const void* virtual_table(
				const RTTI::CompleteObjectLocator* a_col,
				std::span<const std::byte> a_rdata)
			{
		...
		_path(std::move(a_path))
	{
		...
		if (!_image.empty() &&
			...
			_typeInfo = static_cast<const Crash::msvc::type_info*>(v.get());
		}
	}

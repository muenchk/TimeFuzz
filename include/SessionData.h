...
public:
	...
	std::vector<std::shared_ptr<Input>> GetTopK_Length(int32_t k);
	std::vector<std::shared_ptr<Input>> GetTopK_Length_Unfinished(int32_t k);

	std::vector<std::shared_ptr<Input>> FindKSources(int32_t k, std::set<std::shared_ptr<Input>> exclusionlist);
